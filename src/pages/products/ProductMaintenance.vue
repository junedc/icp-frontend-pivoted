  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <div class="max-w-6xl mx-auto p-4">
      ...
      <div class="mt-4 flex gap-3">
        ...
        <button @click="refetch" class="px-4 py-2 rounded-xl border hover:bg-black hover:text-white">Search</button>
      </div>
          ...
          <tbody>
            <tr v-for="p in products" :key="p.id" class="border-t">
              ...
              <td class="p-3 text-right space-x-2">
                ...
                <button class="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 hover:bg-red-50" @click="onDelete(p.id)">
                  Delete
                </button>
              </td>
            </tr>
...
import { fetchProducts, fetchCategories, createProduct, updateProduct, deleteProduct, type Product, type Category } from '@/api/products'
...
const q = useQuery({
  queryKey: ['products', 'maintenance', () => search.value],
...
function openCreate() {
  ...
  Object.assign(form, { name: '', sku: '', price: 0, stock_quantity: 0, category_id: categories.value[0]?.id ?? 0 })
...
function openEdit(p: Product) {
  ...
  Object.assign(form, { name: p.name, sku: p.sku, price: Number(p.price), stock_quantity: p.stock_quantity, category_id: p.category_id })
  modalOpen.value = true
}
...
const saveMutation = useMutation({
  mutationFn: async () => {
    const payload = { name: form.name, sku: form.sku, price: form.price, stock_quantity: form.stock_quantity, category_id: form.category_id }
...
function onSave() { saveMutation.mutate() }
function onDelete(id: number) { if (confirm('Delete this product?')) deleteMutation.mutate(id) }
</script>
