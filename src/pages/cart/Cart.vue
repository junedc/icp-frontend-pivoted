        ...
        <div class="lg:col-span-2 space-y-3">
          ...
          >
            <div class="flex gap-4">
              <!-- IMAGE PLACEHOLDER -->
              <div class="w-20 h-20 bg-slate-200 rounded-lg flex items-center justify-center shrink-0">
              ...
              <div class="flex-1">
                <div class="flex items-start justify-between gap-3">
                  ...
                  <button
                      type="button"
                      class="text-sm px-3 py-1.5 rounded-lg border hover:bg-black hover:text-white"
                ...
                <div class="mt-2 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2 text-xs text-black/60">
                    ...
                    <span
                        class="border rounded-full px-2 py-0.5 whitespace-nowrap"
                        :class="stockBadge(item.stock_quantity).cls"
                    >
      {{ stockBadge(item.stock_quantity).label }}
    </span>
                  </div>
                </div>


                <!-- QTY + STOCK BADGE + LINE TOTAL -->
                <div class="mt-3 flex items-center gap-4">
                  <!-- QTY STEPPER -->
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-black/60">Qty</span>

                    <div class="inline-flex items-center border rounded-xl overflow-hidden">
                      <button
                          type="button"
                          class="px-3 py-2 text-sm hover:bg-black hover:text-white disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-inherit"
                          :disabled="item.quantity <= 1"
                          @click="cart.setQty(item.product_id, item.quantity - 1)"
                      >
                        −
                      </button>

                      <div class="px-4 py-2 text-sm font-medium w-12 text-center select-none">
                        {{ item.quantity }}
                      </div>
        ...
        <div class="border rounded-2xl bg-white p-4 h-fit">
          ...
          <button
              type="button"
              class="mt-4 w-full rounded-xl bg-black text-white py-2"
              :disabled="placeMutation.isPending.value"
              @click="placeOrder"
          >
            {{ placeMutation.isPending.value ? 'Placing...' : 'Place Order' }}
          </button>

          <div v-if="validationError" class="mt-3 text-sm text-red-600">
            {{ validationError }}
          </div>

          <button
              type="button"
              class="mt-3 w-full rounded-xl border py-2 text-sm hover:bg-black hover:text-white"
              @click="cart.clear()"
          >
            Clear Cart
          </button>
